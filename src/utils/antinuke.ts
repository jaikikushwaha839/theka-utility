import { AuditLogEvent, EmbedBuilder, type Guild, type TextChannel, type User } from 'discord.js';
import { AntiNukeConfig, type IAntiNukeConfig } from '../models/AntiNukeConfig';
import { AntiNukeLog, type AntiNukeActionType } from '../models/AntiNukeLog';
 ...
 */
export async function getRecentExecutor(guild: Guild, auditLogType: AuditLogEvent, maxAgeMs = 5000): Promise<User | null> {
    try {
        ...
        return null; // missing "View Audit Log" permission, or a transient API error
    }
}

export async function getAntiNukeConfig(guildId: string): Promise<IAntiNukeConfig | null> {
 ...
 * identified. Skips disabled guilds, the guild owner, whitelisted users, and
 * the bot itself; otherwise counts the action and punishes once the
 * configured threshold is crossed within the configured window.
 */
export async function handleSuspiciousAction({ guild, actor, actionType }: HandleOpts): Promise<void> {
    ...
    const count = recordAndCount(key, cfg.windowSeconds * 1000);
    if (count < cfg.threshold) return;

    actionLog.delete(key); // reset so one punishment doesn't immediately re-trigger
    await punishAndLog(guild, actor, actionType, count, cfg);
}

async function punishAndLog(guild: Guild, actor: User, actionType: AntiNukeActionType, occurrences: number, cfg: IAntiNukeConfig) {
    const member = await guild.members.fetch(actor.id).catch(() => null);
    let punishmentApplied = 'none (member not found — likely already left)';
    ...
    if (member) {
        try {
            if (cfg.punishment === 'ban' && member.bannable) {
                ...
                await member.roles.remove(removable, `Anti-nuke: ${actionType} threshold exceeded`);
                punishmentApplied = `stripped ${removable.size} role(s)`;
            } else {
                punishmentApplied = 'could not act (role hierarchy — this account may outrank the bot)';
            }
        } catch {
            punishmentApplied = 'punishment attempt failed (permissions?)';
        }
    }
    ...
    if (cfg.logChannelId) {
        ...
        if (channel?.isTextBased()) {
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    { name: 'User', value: `${actor.tag} (${actor.id})`, inline: true },
                    { name: 'Action', value: actionType, inline: true },
                    { name: 'Occurrences', value: `${occurrences} within ${cfg.windowSeconds}s`, inline: true },
                    { name: 'Punishment', value: punishmentApplied },
                )
                .setTimestamp();
